...
if page == "Upload & Analyze":
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            # Add message limit selector
            if len(df) > 100:
                st.info(f"📊 Large chat detected ({len(df)} messages). Select how many recent messages to analyze:")
                message_limit = st.slider("Number of messages to analyze", min_value=50, max_value=min(500, len(df)), value=100, step=50)
                st.info(f"💡 Analyzing last {message_limit} messages to avoid API quota limits.")
            else:
                message_limit = len(df)
            
            if st.button("🔍 Generate Summary", type="primary"):
                with st.spinner("Processing chat with AI..."):
                    # Limit the number of messages based on user selection
                    limited_df = df.tail(message_limit) if len(df) > message_limit else df
                    
                    # Convert limited dataframe to string for Gemini
                    chat_history = limited_df.to_string()
                    ...
                    try:
                        ...
                        with col2:
                            # CSV download
                            csv = limited_df.to_csv(index=False)
                            st.download_button(
                        ...
                        if "429" in error_msg or "quota" in error_msg.lower():
                            st.error(f"🚫 API Quota Exceeded: {error_msg}")
                            st.info("💡 Try again in a few minutes, or upload a smaller chat file.")
                        else:
                            st.error(f"❌ Error processing chat file: {error_msg}")
